myks.test.2 <- function (x, y, ..., alternative = c("two.sided", "less", "greater"), 
    exact = NULL) 
{
...
myks.test <- function (x, y, ..., alternative = c("two.sided", "less", "greater"), 
    exact = NULL) 
{
    alternative <- match.arg(alternative)
    DNAME <- deparse(substitute(x))
    ...
    if (is.numeric(y)) {
        ...
        if (length(unique(w)) < (n.x + n.y)) {
            ...
            else warning("p-values will be approximate in the presence of ties")
            z <- z[c(which(diff(sort(w)) != 0), n.x + n.y)]
            TIES <- TRUE
        }
        ...
        nm_alternative <- switch(alternative, two.sided = "two-sided", 
            less = "the CDF of x lies below that of y", greater = "the CDF of x lies above that of y")
        if (exact && (alternative == "two.sided") && !TIES) 
            PVAL <- 1 - .C(stats:::C_psmirnov2x, p = as.double(STATISTIC), 
...
ks.test.2 <- function(samp1, samp2)
{
    
ks <- NULL
result = tryCatch({
    ks <- myks.test(samp1, samp2);   
}, warning = function(w) {
}, error = function(e) {
    if(e$message == "object 'C_psmirnov2x' not found")
        ks <<- myks.test.2(samp1, samp2) else
    print(e)
}, finally = {
})
